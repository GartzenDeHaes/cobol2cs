...
namespace DOR.Core.Data.EF
{
	...
	{
		#region DOR Additions

		/// <summary>
		/// Only works for primative properties
		/// </summary>
		public void ApplyOrginalValues(IObjectWithChangeTracker modelObj)
		{
			Debug.Assert(modelObj.ChangeTracker == this);

			bool old_changeTrackingEnabled = _changeTrackingEnabled;
			...
			if (null != _originalValues)
			{
				foreach (string key in _originalValues.Keys)
				{
					...
					prms[0] = _originalValues[key];
					propInfo.GetSetMethod().Invoke(modelObj, prms);
				}

				AcceptChanges();
			}

			_changeTrackingEnabled = old_changeTrackingEnabled;
		}
	...
	public static class ObjectWithChangeTrackerExtensions
	{
		...
		public static void ApplyOrginalValues(this IObjectWithChangeTracker trackingItem)
		{
			if (trackingItem == null)
			{
				throw new ArgumentNullException("trackingItem");
			...
			trackingItem.ChangeTracker.ApplyOrginalValues(trackingItem);
		}
	}
